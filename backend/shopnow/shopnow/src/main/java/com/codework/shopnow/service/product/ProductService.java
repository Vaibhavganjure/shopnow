...
import com.codework.shopnow.Repository.*;
...
public class ProductService implements IProductService {
    ...
    public Product addProduct(AddProductRequest request) {
        if (productExists(request.getName(), request.getBrand())) {
            throw new EntityExistsException(request.getName() + " Product already exists");
        }
    ...
    public Product updateProduct(ProductUpdateRequest request, Long productId) {
        return productRepository.findById(productId).map(existingProduct -> updateExistingProduct(existingProduct, request)).map(productRepository::save).orElseThrow(() -> new EntityNotFoundException("Product not found"));
    }
    ...
    public List<Product> findDistinctProductByName() {
        ...
        Map<String, Product> distinctProductMap = products.stream().collect((Collectors.toMap(Product::getName, product -> product, (existing, replacement) -> existing)));
